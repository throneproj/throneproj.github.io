...
Команда **не зависит от регистра** (`throne://AddSub` и `throne://addsub` — одно и то же), а завершающая косая черта перед `?` необязательна. В настоящее время Throne поддерживает три команды.

| Команда | Назначение |
| --- | --- |
| `addsub` | Добавить группу подписки и сразу обновить её |
| `route`  | Импортировать профиль маршрутизации |
| `remoteroute` | Добавить один или несколько удалённых профилей маршрутизации по URL |
...
Добавляет один или несколько **удалённых** профилей маршрутизации по URL. В отличие от `route`, который упаковывает весь профиль внутрь ссылки, `remoteroute` содержит ссылки на профили, размещённые в интернете: Throne загружает правила по каждому URL и, если вы разрешите, автоматически поддерживает их в актуальном состоянии. Именно так провайдер передаёт вам поддерживаемый профиль маршрутизации — тот, что улучшается со временем без повторного импорта с вашей стороны.
...
| Параметр | Обязательный | Описание |
| --- | --- | --- |
| `data` | **Да** | JSON-массив записей профилей, закодированный в Base64 (URL-безопасный или стандартный). Обычный, незакодированный JSON также принимается. |

### Что внутри `data`

Полезная нагрузка — это JSON-массив; каждый элемент описывает один удалённый профиль:
...
| --- | --- | --- |
| `url` | **Да** | Ссылка `http://` или `https://` на профиль маршрутизации. Записи без корректного URL http(s) пропускаются. |
| `auto_update` | Нет | Будет ли Throne обновлять этот профиль автоматически. Принимает `true`/`false`, `1`/`0` или `on`/`yes` для включения. При отсутствии **по умолчанию отключено**. |
| `name` | Нет | Отображаемое имя профиля. Если не указано, по умолчанию используется хост `url`. |

> Флаг обновления также может быть записан как `autoUpdate` или `autoupdate`, а всю полезную нагрузку можно обернуть как `{ "routes": [ ... ] }` вместо простого массива.

### Пример

Закодируйте приведённый выше JSON-массив в Base64 и поместите результат в `data`:
...
Throne перечислит всё, что добавит ссылка, и будет ждать вашего подтверждения:

> Добавить эти удалённые профили маршрутизации?
>
> 1. https://example.com/routes/bypass-iran.json  (автообновление: Вкл.)

После подтверждения каждый профиль сразу добавляется и загружается. Профили с включённым автообновлением затем обновляются в фоне согласно интервалу **Routing profiles auto update** в **Базовых настройках (Basic Settings)** (интервал менее 30 минут отключает обновление).
...
- **«Ignored deeplink with unknown command».** Часть с командой — не `addsub`, не `route` и не `remoteroute`. Проверьте написание слова сразу после `throne://`.
...
- **«The link did not contain any valid remote routing profiles».** В ссылке `remoteroute` отсутствовал параметр `data`, он не был JSON-массивом, или ни одна из записей не содержала URL `http://`/`https://`. Убедитесь, что у каждой записи есть корректный URL, а если вы кодировали полезную нагрузку в Base64 — что кодировка не повреждена.
